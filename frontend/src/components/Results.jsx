  ...
  return (
    <div className="space-y-6 mt-8">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg shadow-lg p-6 border border-blue-100">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
            <div className="bg-white rounded-lg p-4 shadow-sm">
              ...
              <div className="text-2xl font-bold text-blue-600">{parsedInsights.class_overview.total_students}</div>
            </div>
            <div className="bg-white rounded-lg p-4 shadow-sm">
              <div className="text-sm text-gray-600">Overall Average</div>
              <div className="text-2xl font-bold text-green-600">{parsedInsights.class_overview.overall_average?.toFixed(1)}%</div>
            </div>
            {parsedInsights.class_overview.performance_distribution && (
              <>
                <div className="bg-white rounded-lg p-4 shadow-sm">
                  <div className="text-sm text-gray-600">Excellent (&gt;90%)</div>
                  <div className="text-2xl font-bold text-purple-600">{parsedInsights.class_overview.performance_distribution.excellent}</div>
                </div>
                <div className="bg-white rounded-lg p-4 shadow-sm">
                  <div className="text-sm text-gray-600">Needs Attention (&lt;60%)</div>
                  <div className="text-2xl font-bold text-orange-600">{parsedInsights.class_overview.performance_distribution.needs_attention}</div>
                </div>
              </>
            )}
          </div>
          ...
          <div className="bg-white rounded-lg p-5 shadow-sm">
            <p className="text-gray-700 leading-relaxed">{parsedInsights.class_overview.summary}</p>
          </div>
        </div>
      )}

      {/* Subject Analysis */}
      {parsedInsights.subject_analysis && parsedInsights.subject_analysis.length > 0 && (
              ...
              <div key={idx} className="bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg p-4 border border-gray-200 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-2">
                  ...
                  <span className={`text-xs px-2 py-1 rounded-full ${
                    subject.trend === 'strength' ? 'bg-green-100 text-green-700' :
                    subject.trend === 'concern' ? 'bg-red-100 text-red-700' :
                ...
                <div className="space-y-1 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold text-blue-600">{subject.class_average?.toFixed(1)}%</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Highest:</span>
                    <span className="font-semibold text-green-600">{subject.highest_score}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Lowest:</span>
                    <span className="font-semibold text-orange-600">{subject.lowest_score}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-600">Difficulty:</span>
                    <span className="font-semibold capitalize">{subject.difficulty_rating}</span>
                  </div>
                </div>
                <p className="text-xs text-gray-600 mt-3 italic">{subject.insight}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* AI Top Performers */}
        {parsedInsights.top_performers && parsedInsights.top_performers.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg p-6 border-t-4 border-green-500">
            ...
            <div className="space-y-3">
              {parsedInsights.top_performers.map((student, idx) => (
                <div key={idx} className="bg-green-50 rounded-lg p-4 hover:bg-green-100 transition-colors">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-lg font-bold text-green-600">{student.overall_percentage?.toFixed(1)}%</span>
                  </div>
                  {student.strengths && student.strengths.length > 0 && (
                    <div className="text-sm text-gray-600 mb-1">
                      <strong>Strengths:</strong> {student.strengths.join(", ")}
                    </div>
                  )}
                  {student.achievement_note && (
                    <div className="text-sm text-gray-700 italic">{student.achievement_note}</div>
                  )}
                </div>
              ))}
            </div>
        ...
        {parsedInsights.students_needing_support && parsedInsights.students_needing_support.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg p-6 border-t-4 border-orange-500">
            ...
            <div className="space-y-3">
              {parsedInsights.students_needing_support.map((student, idx) => (
                <div key={idx} className="bg-orange-50 rounded-lg p-4 hover:bg-orange-100 transition-colors">
                  <div className="flex items-center justify-between mb-2">
                    <span className="font-bold text-gray-800">{student.name}</span>
                    <div className="text-right">
                      <div className="text-lg font-bold text-orange-600">{student.overall_percentage?.toFixed(1)}%</div>
                      <span className={`text-xs px-2 py-1 rounded-full ${
                        student.intervention_priority === 'high' ? 'bg-red-100 text-red-700' : 'bg-yellow-100 text-yellow-700'
                      }`}>
                        {student.intervention_priority} priority
                      </span>
                    </div>
                  </div>
                  {student.critical_subjects && student.critical_subjects.length > 0 && (
                    <div className="text-sm text-gray-600 mb-1">
                      <strong>Critical subjects:</strong> {student.critical_subjects.join(", ")}
                    </div>
                  )}
                  {student.recommended_action && (
                    <div className="text-sm text-gray-700 italic">💡 {student.recommended_action}</div>
                  )}
                </div>
              ))}
            </div>
      ...
      {/* Individual Insights */}
      {parsedInsights.individual_insights && parsedInsights.individual_insights.length > 0 && (
        <div className="bg-white rounded-lg shadow-lg p-6 border-t-4 border-purple-500">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {parsedInsights.individual_insights.map((student, idx) => (
              <div key={idx} className="bg-purple-50 rounded-lg p-4 hover:bg-purple-100 transition-colors">
                <div className="flex items-center justify-between mb-2">
                ...
                {student.strength_subjects && student.strength_subjects.length > 0 && (
                  <div className="text-sm mb-1">
                    <span className="text-green-600 font-semibold">✓ Strengths:</span>
                    <div className="text-gray-700">{student.strength_subjects.join(", ")}</div>
                  </div>
                )}
                {student.improvement_areas && student.improvement_areas.length > 0 && (
                  <div className="text-sm mb-1">
                    <span className="text-orange-600 font-semibold">⚠ Improve:</span>
                    <div className="text-gray-700">{student.improvement_areas.join(", ")}</div>
                  </div>
                )}
                ...
                {student.growth_potential && (
                  <div className="text-xs text-gray-600 mt-2 italic">🌱 {student.growth_potential}</div>
                )}
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {parsedInsights.class_recommendations.immediate_actions && (
              <div className="bg-white rounded-lg p-5 shadow-sm">
                ...
                <ul className="space-y-2">
                  {parsedInsights.class_recommendations.immediate_actions.map((action, idx) => (
            ...
            {parsedInsights.class_recommendations.teaching_strategies && (
              <div className="bg-white rounded-lg p-5 shadow-sm">
                ...
                <ul className="space-y-2">
                  {parsedInsights.class_recommendations.teaching_strategies.map((strategy, idx) => (
                    <li key={idx} className="text-sm text-gray-700 flex items-start gap-2">
            ...
            {parsedInsights.class_recommendations.parent_communication_tips && (
              <div className="bg-white rounded-lg p-5 shadow-sm">
                <h3 className="font-semibold text-gray-700 mb-3 flex items-center gap-2">
                  <span className="text-green-500">👨‍👩‍👧</span> Parent Communication
                </h3>
                <ul className="space-y-2">
                  {parsedInsights.class_recommendations.parent_communication_tips.map((tip, idx) => (
                    <li key={idx} className="text-sm text-gray-700 flex items-start gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {parsedInsights.subject_correlations && parsedInsights.subject_correlations.length > 0 && (
                ...
                <div key={idx} className="bg-teal-50 rounded-lg p-4">
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`px-2 py-1 rounded text-xs font-semibold ${
                      corr.correlation_type === 'positive' ? 'bg-green-200 text-green-800' :
                      corr.correlation_type === 'negative' ? 'bg-red-200 text-red-800' :
                      'bg-gray-200 text-gray-800'
                    }`}>
                      {corr.correlation_type}
                    </span>
                    <span className="text-sm text-gray-600">{corr.subjects?.join(" ↔ ")}</span>
                  </div>
        ...
        {parsedInsights.hidden_insights && parsedInsights.hidden_insights.length > 0 && (
          <div className="bg-white rounded-lg shadow-lg p-6 border-t-4 border-amber-500">
            ...
            <div className="space-y-3">
              {parsedInsights.hidden_insights.map((insight, idx) => (
                <div key={idx} className="bg-amber-50 rounded-lg p-4 flex items-start gap-3">
      ...
      {parsedInsights.motivational_highlights && parsedInsights.motivational_highlights.length > 0 && (
      ...
      {!parsedInsights.class_overview && parsedInsights.summary && (
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg shadow-lg p-6 border border-blue-100">
          ...
          <p className="text-gray-700 leading-relaxed">{parsedInsights.summary}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        {stats.topPerformers && stats.topPerformers.length > 0 && !parsedInsights.top_performers && (
          <div className="bg-white rounded-lg shadow-lg p-6 border-t-4 border-green-500">
            <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
        ...
        {stats.weakSubjects && stats.weakSubjects.length > 0 && !parsedInsights.subject_analysis && (
          <div className="bg-white rounded-lg shadow-lg p-6 border-t-4 border-orange-500">
            <h3 className="text-xl font-bold text-gray-800 mb-4 flex items-center gap-2">
